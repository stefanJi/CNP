...
/*
 * Copy from https://android.googlesource.com/platform/libcore2/+/b5de22c2a7fbe35f0dc8d3585e4bd9aa0966fc90/luni/src/main/java/org/apache/harmony/xnet/provider/jsse/PRF.java
 *
...
public class PRF {
    ...
    static class Logger {
        ...
        public void printAsHex(int i, String s, String s1, byte[] secret) {
            System.out.println(i + s + s1 + Arrays.toString(secret));
        }
    }

    private static Logger logger = new Logger();

    static private void init() throws NoSuchAlgorithmException {
     ...
     */
    static synchronized void computePRF_SSLv3(byte[] out, byte[] secret, byte[] seed) throws NoSuchAlgorithmException {
        if (sha == null) {
            init();
        }
        int pos = 0;
     ...
     * @param seed:      the seed to be used.
     */
    public synchronized static void computePRF(byte[] out, byte[] secret,
                                               byte[] str_bytes, byte[] seed) throws GeneralSecurityException {
        if (sha_mac == null) {
            init();
        }
        ...
        */
        ...
        if (logger != null) {
            ...
            logger.println("label[" + str_bytes.length + "]: ");
